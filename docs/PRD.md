...
By separating the reading experience (display) from the underlying files (data), users can enjoy a rich, stylized "Tome" layout while keeping their Vault files clean, portable, and fully compatible with other plugins or web publishing tools (like Obsidian Publish).

## 2. Core Scope & Features
*   **Custom Reading View:** Introduces a custom Obsidian `ItemView` that renders a stylized, continuous scrolling "Tome" experience.
*   **Pop-Out Window Support:** Fully compatible with Obsidian's native pop-out windows (`app.workspace.getLeaf('window')`), allowing users to read the compiled chronicle on one monitor while editing raw Markdown on another.
